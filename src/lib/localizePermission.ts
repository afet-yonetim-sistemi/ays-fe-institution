...
export const getLocalizedCategory = (category: PermissionCategory, t: (key: string) => string) => {
    const key = permissionCategoryMap[category];
    return t(`permissions.${key}`);
  };
  
  export const getLocalizedPermission = (permission: Permission, t: (key: string) => string) => {
