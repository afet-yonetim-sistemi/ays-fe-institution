...
export const columns: ColumnDef<EmergencyEvacuationApplicationsTableProps>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => row.original.isInPerson ? row.original.firstName : row.original.applicantFirstName,
    size: 170
  },
  {
    accessorKey: 'lastName',
    header: () => i18next.t('applicantLastName'),
    cell: ({ row }) => row.original.isInPerson ? row.original.lastName : row.original.applicantLastName,
    size: 170
  },
  {
    accessorKey: 'phoneNumber',
    header: () => i18next.t('phoneNumber'),
    cell: ({ row }) => row.original.isInPerson ? formatPhoneNumber(row.original.phoneNumber) : formatPhoneNumber(row.original.applicantPhoneNumber),
    size: 170
  },
  {
